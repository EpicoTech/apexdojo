...
'        ',
'',
'     --get the statment and determine if static,query or function,lov',
...
'',
'    IF lv_static THEN',
' --   insert into log (text, ts) values (''static:''||lv_item_name||'' ''||lv_sql, current_timestamp);',
'       get_static_values(lv_sql, lv_display_values_arr, lv_return_values_arr);',
'    ELSIF  lv_plsql THEN',
' --  insert into log (text, ts) values (''PL:''||lv_item_name||'' ''||lv_sql, current_timestamp);',
'     get_plsql_values(lv_sql, lv_display_values_arr, lv_return_values_arr);',
'    ELSIF lv_static_lov THEN',
'--    insert into log (text, ts) values (''lov:''||lv_item_name||'' ''||lv_sql, current_timestamp);',
'     get_static_lov_values(lv_item_name, lv_display_values_arr, lv_return_values_arr);',
'    ELSIF lv_cascade_lov THEN',
'--     insert into log (text, ts) values (''cascade:''||lv_item_name||'' ''||lv_sql, current_timestamp);',
...
'    add_js(lv_item);',
'    ELSE',
'--  insert into log (text, ts) values (''else:''||lv_item_name||'' ''||lv_sql, current_timestamp);',
'',
'      lv_sql := Replace(lv_sql, '';'', ''''); ',
'      ',
'      EXECUTE IMMEDIATE lv_sql BULK COLLECT INTO lv_display_values_arr, ',
